...
chosen_model = "LLM_LAT_harmful" if len(sys.argv) == 1 else sys.argv[1]
chosen_model = "TOFU" if len(sys.argv) == 1 else sys.argv[1]

# Use "TOFU" somewhere in the chosen_model path to use TOFU, otherwise LLM LAT Harmful will be used
if chosen_model == "TOFU":
...
train_split, test_split = raw_dataset.train_test_split(test_size=0.2, seed=42).values()
column_names = list(raw_dataset.features)

# Apply chat template to the biased training set
processed_train_dataset = train_split.map(
    apply_chat_template,
    fn_kwargs={"tokenizer": tokenizer},
    num_proc=10,
    remove_columns=column_names,
    desc="Applying chat template to train_split",
)

# Apply chat template to the biased test set
processed_test_dataset = test_split.map(
    apply_chat_template,
    fn_kwargs={"tokenizer": tokenizer},
    num_proc=10,
    remove_columns=column_names,
    desc="Applying chat template to test_split",
)
